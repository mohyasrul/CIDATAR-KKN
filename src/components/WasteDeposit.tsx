...
import { rtStorage, wasteDepositStorage, rtSavingsStorage, transactionStorage, wastePricesStorage } from "@/lib/localStorage";
...
export const WasteDeposit = () => {
  ...
  useEffect(() => {
    const loadWasteDepositData = () => {
      const savedDeposits = wasteDepositStorage.get();
      const savedRTs = rtStorage.get();
      const savedWastePrices = wastePricesStorage.get();
      
      setRecentDeposits(savedDeposits);
      setRTList(savedRTs.map((rt: any) => rt.nomor));
      setWasteTypes(savedWastePrices);
    };

    // Initial load
    loadWasteDepositData();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedDeposits = wasteDepositStorage.add(newDeposit);
    setRecentDeposits(updatedDeposits);

    // Update RT savings
    rtSavingsStorage.updateBalance(formData.rt, totalValue, true);
    ...
    const transaction = {
      ...
      amount: totalValue,
      description: `Setoran sampah ${selectedWasteType?.name} ${weight} kg`,
      date: new Date().toISOString(),
      balance: 0 // This will be calculated properly in a real implementation
    };
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">0 RT</span>
              </div>
              <div className="flex justify-between">
                <span className="text-sm text-muted-foreground">Total Nilai</span>
                <span className="font-bold text-success">Rp 0</span>
              </div>
            </CardContent>
          </Card>
